...
if (isset($_GET['aksi'])) {
    if ($_GET['aksi'] == "hapus") {
        ...
        if ($res_hapus > 0) {
            Utils::show_alert_redirect("Data berhasil dihapus", 'kelola_warga.php');
        }
    }
}
?>
...
<head>
    ...
    <title>Data Warga | SIPESUTA Kelurahan Kalibaros</title>
    ...
    <div class="drawer drawer-mobile">
        ...
        <div class="drawer-content flex flex-col bg-base-100">
            ...
            <div class="content-body p-4">
                <div class="flex">
                    <?php if($level_staf == 1){ ?>
                    <a href="tambah_ubah_warga.php?aksi=tambah" class="btn btn-primary h-10 min-h-8 text-white my-2"><i class='bx bx-user-plus text-xl mr-1'></i>Tambah Warga</a>
                    <?php } ?>
                    <form action="kelola_warga.php" method="GET" class="ml-auto">
                        <div class="form-control mb-4">
                            <div class="input-group">
                                <input name="search" type="text" placeholder="Cari berdasarkan nama atau email..." class="input input-bordered  h-10 min-h-8 w-80" value="<?= isset($_GET['search']) ? $_GET['search'] : '' ?>"/>
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            if (isset($_GET['search'])) {
                $result = $user->tampil_semua_warga($_GET['search']);
            } else {
                $result = $user->tampil_semua_warga();
            }
            $idx = 1;
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="p-2">
                            <div class="tooltip" data-tip="Edit data #<?= $row['id_user'] ?>">
                                <a href="tambah_ubah_warga.php?aksi=ubah&id=<?= $row['id_user'] ?>" class="btn btn-info h-8 min-h-8 w-8 btn-square text-white" >
                                    <i class='bx bxs-pencil font-medium text-lg'></i>
                                </a>
                            </div>
                            <div class="tooltip" data-tip="Hapus data #<?= $row['id_user'] ?>">
                                <a href="kelola_warga.php?aksi=hapus&id=<?= $row['id_user'] ?>" class="btn btn-error h-8 min-h-8 w-8 btn-square text-white" onclick="return confirm('Anda yakin akan menghapus data #<?= $row['id_user'] ?>')">
